 ...
 * Class Edit View
 *
 * Renders the form for editing an existing class.
 * This view is rendered within the 'admin.php' layout.
 *
 * @var object $class The class object to be edited, passed from ClassController::edit().
 * @var array $courses An array of course objects.
...
$title = "Edit Class: " . htmlspecialchars($class->name);
?>
...
<div class="card shadow-sm">
    <div class="card-body">
        <!-- The form action points to the update route for this specific class -->
        <!-- مسار النموذج يشير إلى رابط التحديث الخاص بهذا الفصل المحدد -->
        <form action="/classes/<?= htmlspecialchars($class->id) ?>/update" method="POST" id="editClassForm">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="semester" name="semester" required>
                        <option value="Fall" <?= ($class->semester === 'Fall') ? 'selected' : '' ?>>Fall</option>
                        <option value="Spring" <?= ($class->semester === 'Spring') ? 'selected' : '' ?>>Spring</option>
                        <option value="Summer" <?= ($class->semester === 'Summer') ? 'selected' : '' ?>>Summer</option>
                        <option value="Winter" <?= ($class->semester === 'Winter') ? 'selected' : '' ?>>Winter</option>
                    </select>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" class="form-control" id="max_students" name="max_students" required min="1"
                           value="<?= htmlspecialchars($class->max_students ?? '30') ?>">
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="schedule" name="schedule" rows="3"><?= htmlspecialchars($class->schedule ?? '') ?></textarea>
            </div>
